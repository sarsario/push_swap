...
/*   Updated: 2023/05/16 14:18:15 by osarsari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	run_cheapest_push(t_stack *src, t_stack *dst, t_plate *plate)
{
	...
	if (!src || !dst || !plate)
		return ;
	dist_from_top = dist_from_top(src, plate);
	dist_from_bot = dist_from_bot(src, plate);
	if (dist_from_top == -1 && dist_from_bot == -1)
		return ;
	if (dist_from_top > dist_from_bot)
		repeat_reverse_rotate(src, dist_from_bot + 1);
	else
		repeat_rotate(src, dist_from_top);
	if (!stack_is_asc(src))
		run_push_stack(src, dst, 'b');
}
